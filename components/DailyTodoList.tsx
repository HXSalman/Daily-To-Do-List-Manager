...
export default function DailyTodoList() {
  ...
  const handleAddTask = () => {
    ...
    setNewTask({ title: "", startDate: new Date(), description: [""], status: "Ongoing" })
                ...
                <Popover>
                  <PopoverTrigger asChild>
                    <Button variant="outline" className="w-full justify-start text-left font-normal mt-1">
                      ...
                      {newTask.startDate ? format(newTask.startDate, "PPP") : <span>Pick a start date</span>}
                    </Button>
                  </PopoverTrigger>
              ...
              <div>
                ...
                <Popover>
                  <PopoverTrigger asChild>
                    <Button variant="outline" className="w-full justify-start text-left font-normal mt-1">
                      <CalendarIcon className="mr-2 h-4 w-4" />
                      {newTask.endDate ? format(newTask.endDate, "PPP") : <span>Pick an end date</span>}
                    </Button>
                  </PopoverTrigger>
                  <PopoverContent className="w-auto p-0">
                    <Calendar
          ...
          <Card key={task.id} className={`${getStatusColor(task.status)} border-l-4 shadow-md transition-all hover:shadow-lg`}>
            ...
            <CardContent>
              <p className="text-sm mb-2">
                <span className="font-semibold">Start:</span> {format(task.startDate, "PPP")}
                {task.endDate && <><br /><span className="font-semibold">End:</span> {format(task.endDate, "PPP")}</>}
              </p>
              ...
              <p className="text-sm font-semibold mt-2">Status: {task.status}</p>
            </CardContent>
          </Card>
        ))}
      </div>
